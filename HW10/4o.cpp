...
// }
PII get(int k, int nl, int nr, int l, int r){
  if(r < nl || l > nr) return PII((int)1e9,0);
  if(l <= nl && nr <= r) return val[k];	// return answer here
  int nm = (nl+nr)/2;
  return min(get(2*k+2, nm+1, nr, l, r), get(2*k+1, nl, nm, l, r));
}
...
void dfs(int x){
  REP(i,tree[x].ns.size()){
    ...
    if(!visited[y]){
      ...
      //P4("x:",dis[x],"y:",dis[y]);
      occur[y] = seg_count;
      arr[seg_count] = PII(depth[y], y);
      seg_count++;
      dfs(y);
      arr[seg_count] = PII(depth[x], x);
      seg_count++;
    }
  }
}
...
signed main(){
  ...
  REP(i,q){
    ...
    int c = get(0,0,seg_count-1,MIN(occur[a],occur[b]),MAX(occur[a],occur[b])).second;
